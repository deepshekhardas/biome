//! This is a generated file. Don't modify it by hand! Run 'cargo codegen formatter' to re-generate the file.

use crate::prelude::*;
use biome_css_syntax::AnyCssAttributeMatcherValue;
use biome_formatter::FormatRuleWithOptions;
#[derive(Debug, Clone, Default)]
pub(crate) struct FormatAnyCssAttributeMatcherValue {
    case: CssCase,
}
impl FormatRuleWithOptions<AnyCssAttributeMatcherValue> for FormatAnyCssAttributeMatcherValue {
    type Options = CssCase;
    fn with_options(mut self, options: Self::Options) -> Self {
        self.case = options;
        self
    }
}
impl FormatRule<AnyCssAttributeMatcherValue> for FormatAnyCssAttributeMatcherValue {
    type Context = CssFormatContext;
    fn fmt(&self, node: &AnyCssAttributeMatcherValue, f: &mut CssFormatter) -> FormatResult<()> {
        match node {
            AnyCssAttributeMatcherValue::CssIdentifier(node) => {
                node.format().with_case(self.case).fmt(f)
            }
            AnyCssAttributeMatcherValue::CssString(node) => node.format().fmt(f),
            AnyCssAttributeMatcherValue::ScssInterpolatedIdentifier(node) => node.format().fmt(f),
            AnyCssAttributeMatcherValue::ScssInterpolatedString(node) => node.format().fmt(f),
        }
    }
}
